...
void BitblastSolver::preRegister(TNode node) {
  if ((node.getKind() == kind::EQUAL ||
    ...
    if ((options::decisionUseWeight() || options::decisionThreshold() != 0) &&
      ...
      decision::DecisionWeight nodeWeight = d_bitblaster->computeAtomWeight(node);
      node.setAttribute(theory::DecisionWeightAttr(), nodeWeight);
      Trace("decision::bvweight") << nodeWeight << " ";
      Trace("decision::bvweightfull") << node << std::endl;
    }
  }
}
